 ...
 * Models
 */
gltfLoader.load(`/models/toilet_1.glb`, (gltf) => {
  toilet[0] = gltf.scene
...
gltfLoader.load("/models/cursor.glb", (gltf) => {
  ...
  cursor.position.set(0, 1.76, 0.3)
...
window.addEventListener("mousedown", (event) => {
  ...
  if (intersects.length > 0) {
    ...
    if (pointedObject.name === "Hidden" && isInteractAvailable) {
...
const animate = () => {
  ...
  if (waterFlow[0]) {
    waterFlow[0].map.offset.x += 0.01
  }

  if (mixer[1]) mixer[1].update(deltaTime)
  if (waterFlow[1]) {
    waterFlow[1].map.offset.x += 0.01
  }
